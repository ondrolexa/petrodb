...
from petroapi.models import User, Project, Sample, Area
...
# CREATE Sample Area
@router.post("/areas/{project_id}/{sample_id}", response_model=AreaSchema)
def create_area(
    project_id: int,
    sample_id: int,
    ...
    if db.query(Area).filter_by(sample_id=sample_id).filter_by(label=area.label).first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Area with same label already exists"
        )
...
@router.get("/areas/{project_id}/{sample_id}/{area_id}")
def get_area(
    project_id: int,
    sample_id: int,
    area_id: int,
...
# UPDATE Sample Area
@router.put(
    "/areas/{project_id}/{sample_id}/{area_id}", response_model=AreaSchema
)
def update_area(
    project_id: int,
    sample_id: int,
...
# DELETE Sample Area
@router.delete(
    "/areas/{project_id}/{sample_id}/{area_id}", response_model=dict[str, str]
)
def delete_area(
    project_id: int,
    sample_id: int,
