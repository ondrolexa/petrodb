...
# CREATE Sample Spot
@router.post("/spots/{project_id}/{sample_id}", response_model=SpotSchema)
def create_spot(
    project_id: int,
    sample_id: int,
    ...
    if db.query(Spot).filter_by(sample_id=sample_id).filter_by(label=spot.label).first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Spot with same label already exists"
        )
...
@router.get("/spots/{project_id}/{sample_id}/{spot_id}")
def get_spot(
    project_id: int,
    sample_id: int,
    spot_id: int,
...
# UPDATE Sample Spot
@router.put(
    "/spots/{project_id}/{sample_id}/{spot_id}", response_model=SpotSchema
)
def update_spot(
    project_id: int,
    sample_id: int,
...
# DELETE Sample Spot
@router.delete(
    "/spots/{project_id}/{sample_id}/{spot_id}", response_model=dict[str, str]
)
def delete_spot(
    project_id: int,
    sample_id: int,
